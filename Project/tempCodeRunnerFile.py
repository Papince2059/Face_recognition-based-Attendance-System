...
class FaceRecognition:
    def __init__(self, root):
        ...
        # Load YOLOv8 Model
        self.model = YOLO("yolov8n-face.pt")  # Ensure you have a YOLO model for face detection
    ...
    def mark_attendance(self, name):
        ...
        if name in self.marked_students:
            messagebox.showinfo("Attendance", f"Attendance already marked for {name}. Camera shutting down.")
            self.shutdown_camera()
            return
    ...
    def face_recognize(self):
        ...
        while True:
            ...
            results = self.model(frame)  # YOLOv8 face detection
            for result in results:
                for box in result.boxes.xyxy:
                    x1, y1, x2, y2 = map(int, box[:4])
                    face_img = frame[y1:y2, x1:x2]  # Crop detected face
